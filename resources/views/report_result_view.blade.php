...
@section('content')
    ...
    <div class="container">
        ...
        <div class="viewer-title">
            <h3>{{$report->report_name}} <span class="badge bg-info">{{$response->total}} Items</span></h3>
        ...
        <table class="table table-responsive">
            <thead class="table-light">
                <th scope="col"><a href="/report/run/{{$report->id}}?sortby=manufacturer_name">Software Manufacturer</a></th>
                <th scope="col"><a href="/report/run/{{$report->id}}?sortby=software_name">Software Package</a></th>
                <th scope="col"><a href="/report/run/{{$report->id}}?sortby=software_version">Software Version</a></th>
                <th scope="col"><a href="/report/run/{{$report->id}}?sortby=managed_sw_id">Managed Software ID</a></th>
                <th scope="col"><a href="/report/run/{{$report->id}}?sortby=human_readable_detected_time">Last Detected</a></th>
                <th scope="col"><a href="/report/run/{{$report->id}}?sortby=network_installations">Installations</a></th>
            </thead>
            <tbody>
        ...
        @else
        <div class="viewer-title">
            <h3>{{$report->report_name}} <span class="badge bg-info">{{$response->total}} Devices</span></h3>
            <div class="list-links">
                <p>Query Items Limit: {{$response->limit}} | Page Number: {{$response->page}}</p>
            </div>
        </div>
